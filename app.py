...
if st.button("Predict Stroke Risk"):
    ...
    with st.expander("Logistic Regression SHAP Interpretation", expanded=False):
        explainer1 = shap.Explainer(logistic_regression_model)
        shap_values1 = explainer1(input_data)
        fig1, ax1 = plt.subplots()
        shap.summary_plot(shap_values1, input_data, plot_type="bar", show=False)
        st.pyplot(fig1)
        
    with st.expander("Naive Bayes SHAP Interpretation", expanded=False):
        explainer2 = shap.Explainer(naive_bayes_model)
        shap_values2 = explainer2(input_data)
        fig2, ax2 = plt.subplots()
        shap.summary_plot(shap_values2, input_data, plot_type="bar", show=False)
        st.pyplot(fig2)

    with st.expander("SVM SHAP Interpretation", expanded=False):
        explainer3 = shap.Explainer(svm_model)
        shap_values3 = explainer3(input_data)
        fig3, ax3 = plt.subplots()
        shap.summary_plot(shap_values3, input_data, plot_type="bar", show=False)
        st.pyplot(fig3)

    with st.expander("Decision Tree SHAP Interpretation", expanded=False):
        explainer4 = shap.Explainer(decision_tree_model)
        shap_values4 = explainer4(input_data)
        fig4, ax4 = plt.subplots()
        shap.summary_plot(shap_values4, input_data, plot_type="bar", show=False)
        st.pyplot(fig4)

    with st.expander("Random Forest SHAP Interpretation", expanded=False):
        explainer5 = shap.Explainer(random_forest_model)
        shap_values5 = explainer5(input_data)
        fig5, ax5 = plt.subplots()
        shap.summary_plot(shap_values5, input_data, plot_type="bar", show=False)
        st.pyplot(fig5)
